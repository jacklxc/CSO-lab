 ...
 *
 * Explicit free list
 * Pointers, headers, and footers are 64 bits = 8 bytes = long = double words
 */
...
int mm_init(void) {
    ...
    PUT(HDRP(heap_start), PACK(DWORD, 1));
    PUT(FTRP(heap_start), PACK(DWORD, 1));
    //set epilogue
...
static void *extend_heap(size_t words) {
    ...
    //coalesce and return the block pointer
    return coalesce(bp);
}
...
static void insert_to_flist(void *bp) {
    ...
    //operations for if the free list isn't empty
    if (flist_head != NULL) {
...
static void remove_block(void *bp) {
    ...
    //fix bp's previous pointer if there is a next block
    if (NEXT_FREE(bp))
